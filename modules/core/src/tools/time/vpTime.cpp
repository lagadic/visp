...
double measureTimeMs()
{
  ...
  throw(vpException(
      ...
      "Cannot get time: not implemented on Universal Windows Platform"));
#endif
#elif !defined(_WIN32) &&                                                    \
...
int wait(double t0, double t)
{
  ...
  if (timeToWait <= 0.) // no need to wait
    ...
    if (timeToWait > vpTime::minTimeForUsleepCall) {
      usleep(
          (useconds_t)((timeToWait - vpTime::minTimeForUsleepCall) * 1000));
    }
...
/*!
   ...
   \param[in] format : The string format supported by strftime() function that
represents the time. The default format is the following \c "%Y/%m/%d
%H:%M:%S". This string contains any combination of special format specifiers
given in the next table: | specifier | Replaced by
| Example                  |
   |-----------|------------------------------------------------------|--------------------------|
   | %%a	     | Abbreviated weekday name *                           | Thu
| | %%A	     | Full weekday name *                                  |
Thursday                 | | %%b	     | Abbreviated month name *
| Aug                      | | %%B	     | Full month name *
| August                   | | %%c	     | Date and time representation *
...
02:55:02 pm              | | %%R       | 24-hour HH:MM time, equivalent to
%H:%M              | 14:55                    | | %%S       | Second (00-61)
| 02                       | | %%t       | Horizontal-tab character ('\\t')
|                          | | %%T       | ISO 8601 time format (HH:MM:SS),
equivalent to %H:%M:%S | 14:55:02              | | %%u       | ISO 8601
weekday as number with Monday as 1 (1-7)    | 4                        | | %%U
...
(00-53) | 34     | | %%x       | Date representation *
| 08/23/01                 | | %%X       | Time representation *
| 14:55:02                 | | %%y       | Year, last two digits (00-99)
| 01                       | | %%Y       | Year
| 2001                     | | %%z       | ISO 8601 offset from UTC in
timezone (1 minute=1, 1 hour=100) \n If timezone cannot be determined, no
characters	| +100 | | %%Z       | Timezone name or abbreviation * \n If
timezone cannot be determined, no characters| CDT | | %%        | A % sign
|	%                        |
...
int main()
{
  std::cout << "%Y/%m/%d %H:%M:%S (default): " << vpTime::getDateTime() <<
std::endl; std::cout << "%Y-%m-%d_%H.%M.%S format   : " <<
vpTime::getDateTime("%Y-%m-%d_%H.%M.%S") << std::endl; std::cout << "%F
format   : " << vpTime::getDateTime("%F") << std::endl; std::cout << "%X
format   : " << vpTime::getDateTime("%X") << std::endl;
