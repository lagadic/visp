...
/*!
  ...
  \param msg : ASCII message or byte data according to the data sent by the client.
  \param timeoutMs : Timeout in millisecond (if zero, the call is blocking).

  \return The message length / size of the byte array sent received, or -1 if there is an error,
  or 0 if there is a timeout.
...
/*!
  ...
  \param msg : ASCII message or byte data according to the data send by the client.
  \param hostInfo : Information about the client ("client_name client_ip client_port").
  \param timeoutMs : Timeout in millisecond (if zero, the call is blocking).
...
#if !defined(_WIN32) && (defined(__unix__) || defined(__unix) || (defined(__APPLE__) && defined(__MACH__))) // UNIX
    int length = recvfrom(m_socketFileDescriptor, m_buf, sizeof(m_buf), 0, (struct sockaddr *) &m_clientAddress, (socklen_t *)&m_clientLength);
#else
    int length = recvfrom(m_socketFileDescriptor, m_buf, sizeof(m_buf), 0, (struct sockaddr *) &m_clientAddress, &m_clientLength);
#endif
    if (length <= 0) {
      return length < 0 ? -1 : 0;
    }
...
#if !defined(_WIN32) && (defined(__unix__) || defined(__unix) || (defined(__APPLE__) && defined(__MACH__))) // UNIX
  return sendto(m_socketFileDescriptor, msg.c_str(), msg.size(), 0, (struct sockaddr *) &m_clientAddress, m_clientLength);
#else
  return sendto(m_socketFileDescriptor, msg.c_str(), (int) msg.size(), 0, (struct sockaddr *) &m_clientAddress, m_clientLength);
#endif
}
