 ...
 *
 * Example of client's code, receiving and sending basic message
 ...
 * \endcode
 *
 * Example of client's code, sending a vpImage on request form.
...
{
private:
  unsigned int m_numberOfAttempts;

  bool connectServer(vpNetwork::vpReceptor &serv);

public:
  vpClient();
  virtual ~vpClient() override;
   ...
   */
  unsigned int getNumberOfAttempts() { return m_numberOfAttempts; }
   ...
   */
  void setNumberOfAttempts(const unsigned int &nb) { m_numberOfAttempts = nb; }
