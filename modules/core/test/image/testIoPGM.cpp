...
// Set the default output path
    ...
    try {
      ...
      filename =
          vpIoTools::createFilePath(ipath, "image-that-does-not-exist.pgm");
      std::cout << "Read image: " << filename << std::endl;
      vpImageIo::read(I, filename);
    } catch (vpException &e) {
      std::cout << "Catch an exception due to a non existing file: " << e
                << std::endl;
    }
    ...
    try {
      ...
      filename = vpIoTools::createFilePath(
          opath, "directory-that-does-not-exist/Klimt.pgm");
      std::cout << "Write image: " << filename << std::endl;
      vpImageIo::write(I, filename);
    } catch (vpException &e) {
      std::cout << "Catch an exception due to a non existing file: " << e
