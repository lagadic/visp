...
int main()
{
  try {
    ...
    if (!vpMath::equal(q3.x(), 2.6, std::numeric_limits<double>::epsilon()) ||
        ...
        !vpMath::equal(q3.z(), -3.0,
                       std::numeric_limits<double>::epsilon()) ||
        !vpMath::equal(q3.w(), 4.0, std::numeric_limits<double>::epsilon())) {
      throw vpException(vpException::fatalError,
                        "Problem with addition of two quaternions !");
    }

    // Test subtraction of two quaternions
    vpQuaternionVector q4 = q3 - q1;
    std::cout << "q4=" << q4 << std::endl;
    if (!vpMath::equal(q4.x(), q2.x(),
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q4.y(), q2.y(),
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q4.z(), q2.z(),
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q4.w(), q2.w(),
                       std::numeric_limits<double>::epsilon() * 1e4)) {
      throw vpException(vpException::fatalError,
                        "Problem with subtraction of two quaternions !");
    }
    ...
    if (!vpMath::equal(q7.x(), 3.0,
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q7.y(), 36.7,
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q7.z(), -6.6,
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q7.w(), 1.3,
                       std::numeric_limits<double>::epsilon() * 1e4)) {
      throw vpException(vpException::fatalError,
                        "Problem with multiplication of two quaternions !");
    }

    // Test quaternion conjugate
    vpQuaternionVector q7_conj = q7.conjugate();
    std::cout << "q7_conj=" << q7_conj << std::endl;
    if (!vpMath::equal(q7_conj.x(), -3.0,
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q7_conj.y(), -36.7,
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q7_conj.z(), 6.6,
                       std::numeric_limits<double>::epsilon() * 1e4) ||
        !vpMath::equal(q7_conj.w(), 1.3,
                       std::numeric_limits<double>::epsilon() * 1e4)) {
      throw vpException(vpException::fatalError,
                        "Problem with quaternion conjugate !");
    }
    ...
    if (!vpMath::equal(q7_inv.x(), -0.00214111, 0.000001) ||
      ...
      throw vpException(vpException::fatalError,
                        "Problem with quaternion inverse !");
    }
    ...
    if (!vpMath::equal(q7_norm, 37.4318, 0.0001)) {
      throw vpException(vpException::fatalError,
                        "Problem with quaternion magnitude !");
    }
    ...
    if (!vpMath::equal(q7.x(), 0.0801457, 0.00001) ||
      ...
      throw vpException(vpException::fatalError,
                        "Problem with quaternion normalization !");
    }
