...
int main() {
  ...
  std::vector<vpThread> threads((size_t) nb_threads);
  std::vector<ArithmFunctor> functors((size_t) nb_threads);
  unsigned int split = size / nb_threads;
  for (unsigned int i = 0; i < nb_threads; i++) {
    if (i < nb_threads-1) {
      functors[(size_t) i] = ArithmFunctor(v1, v2, i*split, (i+1)*split);
    } else {
      functors[(size_t) i] = ArithmFunctor(v1, v2, i*split, size);
    }

    threads[(size_t) i] = vpThread((vpThread::Fn) arithmThread, (vpThread::Args) &functors[i]);
  }
  ...
  for (unsigned int i = 0; i < nb_threads; i++) {
    ...
    insert(res_add, functors[(size_t) i].getVectorAdd());
    insert(res_mul, functors[(size_t) i].getVectorMul());
  }
