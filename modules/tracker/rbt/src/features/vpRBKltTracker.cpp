...
void vpRBKltTracker::extractFeatures(const vpRBFeatureTrackerInput &frame, const vpRBFeatureTrackerInput &previousFrame, const vpHomogeneousMatrix &cMo)
{
  ...
  if (m_maxErrorOutliersPixels > 0.0) {
    double distanceThresholdPxSquare = vpMath::sqr(m_maxErrorOutliersPixels);
    // Detect outliers
    for (unsigned int i = 0; i < m_klt.getNbFeatures(); ++i) {
      long id;
      float u, v;
      double x, y;
      ...
      if (m_points.find(id) != m_points.end()) {
        ...
        if (Z > 0.f) {
          ...
          if (distancePx > m_maxErrorOutliersPixels) {
            m_points.erase(id);
  ...
  double timeBeforeMask = vpTime::measureTimeMs();
  for (unsigned int i = bb.getTop(); i < bb.getBottom(); ++i) {
    for (unsigned int j = bb.getLeft(); j < bb.getRight(); ++j) {
  ...
  if (m_Iprev.rows > 0) {
    ...
    if (m_points.size() < m_numPointsReinit) {
      ...
      for (unsigned int i = 0; i < kltTemp.getNbFeatures(); ++i) {
        ...
        for (unsigned int j = 0; j < m_klt.getNbFeatures(); ++j) {
          float uj, vj;
          long idj;
...
{
  for (const std::pair<long, vpTrackedKltPoint> &p : m_points) {
    double u, v;
