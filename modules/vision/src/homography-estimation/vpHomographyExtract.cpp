...
/**** Corps de la focntion ****/
  ...
  *****/
  sinusTheta = signeSinus *
               sqrt((sv[0] * sv[0] - sv[1] * sv[1]) *
                    (sv[1] * sv[1] - sv[2] * sv[2])) /
               ((sv[0] + sv[2]) * sv[1]);
...
void vpHomography::computeDisplacement(const vpHomography &H, const double x,
                                       const double y,
  ...
  if (distanceFictive < 0) {
    ...
    if (cas == cas1) {
      if (norm1ok) {
        ...
        // cos theta
        Rprim1[0][0] =
            (sv[0] * sv[2] - vpMath::sqr(sv[1])) / ((sv[0] - sv[2]) * sv[1]);
