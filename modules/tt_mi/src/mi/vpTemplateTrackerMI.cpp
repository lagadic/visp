...
void vpTemplateTrackerMI::setBspline(const vpBsplineType &newbs)
{
  ...
  Pt= new double[Ncb];
	...
	dPrt= new double[Ncb*Ncb*nbParam];
	d2Prt= new double[Ncb*Ncb*nbParam*nbParam];

	/*std::cout<<Nc*Nc*influBspline<<std::endl;std::cout<<Nc*Nc*nbParam*influBspline<<std::endl;*/	
  PrtD= new double[Nc*Nc*influBspline];
	dPrtD= new double[Nc*Nc*nbParam*influBspline];
    PrtTout= new double[Nc*Nc*influBspline*(1+nbParam+nbParam*nbParam)];
...
void vpTemplateTrackerMI::setNc(int nc)
{
  ...
  if (PrtTout) delete[] PrtTout;
	...
	Pt= new double[Ncb];
	Pr= new double[Ncb];
	d2Prt= new double[Ncb*Ncb*nbParam*nbParam];//(r,t)
    PrtTout= new double[Nc*Nc*influBspline*(1+nbParam+nbParam*nbParam)];
}
...
void vpTemplateTrackerMI::computeHessien(vpMatrix &H)
{
	...
	for(int t=0;t<Ncb;t++)
	{
		...
		if(Pt[t]>seuilevitinf)
		{
			...
			{
				//if(Prt[r*Ncb+t]!=0)
				if(Prt[r*Ncb+t]>seuilevitinf)
				{
					for(int it=0;it<nbParam;it++)
						dprtemp[it]=dPrt[(r*Ncb+t)*nbParam+it];
						
					dtemp=1.+log(Prt[r*Ncb+t]/Pt[t]);
					for(int it=0;it<nbParam;it++)
						for(int jt=0;jt<nbParam;jt++)
							if(ApproxHessian!=HESSIAN_NONSECOND && ApproxHessian!=HESSIAN_NEW)
                                H[it][jt]+=dprtemp[it]*dprtemp[jt]*(1./Prt[r*Ncb+t]-1./Pt[t])+d2Prt[(r*Ncb+t)*nbParam*nbParam+it*nbParam+jt]*dtemp;
                            else if(ApproxHessian==HESSIAN_NEW)
                                H[it][jt]+=d2Prt[(r*Ncb+t)*nbParam*nbParam+it*nbParam+jt]*dtemp;
                            else
                                H[it][jt]+=dprtemp[it]*dprtemp[jt]*(1./Prt[r*Ncb+t]-1./Pt[t]);
            ...
            if(Pt[t]>seuilevitinf)
            {
                    for(int r=0;r<Ncb;r++)
                    {
                            ...
                            if(Prt[r*Ncb+t]>seuilevitinf)
                            {
                                    ...
                                    dtemp=1.+log(Prt[r*Ncb+t]/Pt[t]);
                                    // u = som(Pxy.logPxPy)
                                    u += Prt[r*Ncb+t]*log(Pt[t]*Pr[r]);
                                    // v = som(Pxy.logPxy)
                                    v += Prt[r*Ncb+t]*log(Prt[r*Ncb+t]);

                                    for(int it=0;it<nbParam;it++){
                                        // u' = som dPxylog(PxPy)
                                        du[it] += dprtemp[it]*log(Pt[t]*Pr[r]);
                                        // v' = som dPxy(1+log(Pxy))
                                        dv[it] += dprtemp[it]*(1+log(Prt[r*Ncb+t]));

                                    }
                                    for(int it=0;it<nbParam;it++){
                                            for(int jt=0;jt<nbParam;jt++){
                                                d2u[it][jt] += d2Prt[(r*Ncb+t)*nbParam*nbParam+it*nbParam+jt]*log(Pt[t]*Pr[r])
                                                        + (1.0/Prt[r*Ncb+t])*(dprtemp[it]*dprtemp[it]);
                                                d2v[it][jt] += d2Prt[(r*Ncb+t)*nbParam*nbParam+it*nbParam+jt]*(1+log(Prt[r*Ncb+t]))
                                                        + (1.0/Prt[r*Ncb+t])*(dprtemp[it]*dprtemp[it]);
                                            }
                                    }
                            }
...
{
	for(int i=0;i<Ncb*Ncb;i++)Prt[i]=0;
	for(int i=0;i<Ncb*Ncb*nbParam;i++)dPrt[i]=0;
	for(int i=0;i<Ncb*Ncb*nbParam*nbParam;i++)d2Prt[i]=0;
    for(int i=0;i<Nc*Nc*influBspline*(1+nbParam+nbParam*nbParam);i++)PrtTout[i]=0.0;
...
double vpTemplateTrackerMI::getMI256(vpImage<unsigned char> &I,vpColVector &tp)
{
  ...
  for(int point=0;point<templateSize;point++)
  {
    ...
    if((i2>=0)&&(j2>=0)&&(i2<I.getHeight()-1)&&(j2<I.getWidth())-1)
    {
      ...
      Tij=ptTemplate[point].val;
      if(!blur)
        IW=(int)I.getValue(i2,j2);
      else
        IW=(int)GaussI.getValue(i2,j2);
