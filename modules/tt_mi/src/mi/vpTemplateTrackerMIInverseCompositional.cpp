...
{
    ptTemplateSupp[ptIndex].BtInit=new double[(1 + nbParam + nbParam*nbParam)*bspline];
...
//    std::cout << "NMI avant: " << NMI_preEstimation << std::endl;
    ...
    int totParam = (bspline * bspline)*(1+nbParam+nbParam*nbParam);
    int size = (1 + nbParam + nbParam*nbParam)*bspline;

    #if defined(USE_OPENMP_MI_INVCOMP) && defined(DECSA_HAVE_OPENMP)
    int sizePrtBis = Ncb*Ncb;
    int sizedPrtBis = Ncb*Ncb*nbParam;
    int sized2PrtBis = Ncb*Ncb*nbParam*nbParam;
    #endif
