 ...
 * For example if a vpFeatureMomentCInvariant is duplicated, the duplicate will
 ...
 * documentation of the specialized vpFeatureMoment classes when deciding to
...
public:
   ...
   * primitives, the object plane, feature database and matrix size.
   * \param data_base : Moment database. The database of moment primitives (first
   * parameter) is mandatory. It is used to access different moment values later
   * used to compute the final matrix.
   ...
   * \param nbmatrices : If you want to create a new vpFeatureMoment implementation,
