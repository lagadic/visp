...
# the list only for debugging. The real list, used in the real ViSP build, is specified in CMakeLists.txt
visp_hdr_list = [
    "../../core/include/visp3/core/vpMath.h"
    # TODO Contains only 1 file for now. Add other files and modules later. Refer the OpenCV equivalent for that
]
...
class CppHeaderParser(object):
    ...
    def parse_arg(self, arg_str, argno):
        ...
        if "VP_OUT" in arg_str:
            modlist.append("/O")
            arg_str = arg_str.replace("VP_OUT", "")

        if "VP_IN_OUT" in arg_str:
            modlist.append("/IO")
            arg_str = arg_str.replace("VP_IN_OUT", "")

        isarray = False
        npos = arg_str.find("VP_CARRAY")
        if npos >= 0:
            isarray = True
        ...
        npos = arg_str.find("VP_CUSTOM_CARRAY")
        if npos >= 0:
            isarray = True
            macro_arg, npos3 = self.get_macro_arg(arg_str, npos)
        ...
        arg_type = self.batch_replace(arg_type, [("std::", ""), ("vp::", ""), ("::", "_")])
    ...
    def parse_class_decl(self, decl_str):
        ...
        Parses class/struct declaration start in the form:
           {class|struct} [VISP_EXPORT] <class_name> [: public <base_class1> [, ...]]
        Returns class_name1, <list of base_classes>
        """
        l = decl_str
        modlist = []
        if "VISP_EXPORT_W_MAP" in l:
            l = l.replace("VISP_EXPORT_W_MAP", "")
            modlist.append("/Map")
        if "VISP_EXPORT_W_SIMPLE" in l:
            l = l.replace("VISP_EXPORT_W_SIMPLE", "")
            modlist.append("/Simple")
        npos = l.find("VISP_EXPORT_AS")
        if npos >= 0:
            macro_arg, npos3 = self.get_macro_arg(l, npos)
            modlist.append("=" + macro_arg)
            l = l[:npos] + l[npos3 + 1:]

        l = self.batch_replace(l, [("VISP_EXPORT_W", ""), ("VISP_EXPORT", ""), ("public virtual ", " "), ("public ", " "),
                                   ("::", ".")]).strip()
    ...
    def parse_func_decl_no_wrap(self, decl_str, static_method=False, docstring=""):
        ...
        fdecl = decl_str.replace("VP_OUT", "").replace("VP_IN_OUT", "")
        ...
        if args0str != "" and args0str != "void":
            ...
            for arg in args:
                ...
                if dfpos >= 0:
                    ...
                    dfpos = arg.find("VP_DEFAULT")
                    if dfpos >= 0:
                        defval, pos3 = self.get_macro_arg(arg, dfpos)
                    else:
                        dfpos = arg.find("VP_WRAP_DEFAULT")
                        if dfpos >= 0:
                            defval, pos3 = self.get_macro_arg(arg, dfpos)
                if dfpos >= 0:
    ...
    def parse_func_decl(self, decl_str, use_umat=False, docstring=""):
        ...
        [([VISP_EXPORT] <rettype>) | VPAPI(rettype)]
            [~]<function_name>
        ...
        if self.wrap_mode:
            if not (("VISP_EXPORT_AS" in decl_str) or ("VISP_EXPORT_W" in decl_str) or \
                    ("VP_WRAP" in decl_str) or ("VP_WRAP_AS" in decl_str)):
                return []

        # ignore old API in the documentation check (for now)
        if "VPAPI(" in decl_str and self.wrap_mode:
            return []

        top = self.block_stack[-1]
        func_modlist = []

        npos = decl_str.find("VISP_EXPORT_AS")
        if npos >= 0:
            arg, npos3 = self.get_macro_arg(decl_str, npos)
            func_modlist.append("=" + arg)
            decl_str = decl_str[:npos] + decl_str[npos3 + 1:]
        npos = decl_str.find("VP_WRAP_AS")
        if npos >= 0:
            arg, npos3 = self.get_macro_arg(decl_str, npos)
            func_modlist.append("=" + arg)
            decl_str = decl_str[:npos] + decl_str[npos3 + 1:]
        ...
        decl_str = self.batch_replace(decl_str, [("static inline", ""), ("inline", ""), \
                                                 ("VISP_EXPORT_W", ""), ("VISP_EXPORT", ""), ("VP_CDECL", ""),
                                                 ("VP_WRAP ", " "), ("VP_INLINE", ""),
                                                 ("VP_DEPRECATED", "")]).strip()
        ...
        if decl_str.startswith("VPAPI"):
            rtype_end = decl_str.find(")", args_begin + 1)
            if rtype_end < 0:
                print("Error at %d. no terminating ) in VPAPI() macro: %s" % (self.lineno, decl_str))
                sys.exit(-1)
            ...
            if (t == "," and balance == 1 and angle_balance == 0) or balance == 0:
                ...
                if a:
                    ...
                    if eqpos >= 0:
                        ...
                        eqpos = a.find("VP_DEFAULT")
                        if eqpos >= 0:
                            defval, pos3 = self.get_macro_arg(a, eqpos)
                        else:
                            eqpos = a.find("VP_WRAP_DEFAULT")
                            if eqpos >= 0:
                                defval, pos3 = self.get_macro_arg(a, eqpos)
                    if defval == "NULL":
                        defval = "0"
    ...
    def get_dotted_name(self, name):
        ...
        the function will convert "A" to "vp.A" and "f" to "vp.A.f".
        """
    ...
    def parse_stmt(self, stmt, end_token, use_umat=False, docstring=""):
            ...
            if stmt.startswith("class") or stmt.startswith("struct"):
                ...
                if stmt.strip() != stmt_type:
                    try:
                        ...
                        exit(1)
                    decl = []
                    if ("VISP_EXPORT_W" in stmt) or ("VISP_EXPORT_AS" in stmt) or (
                    not self.wrap_mode):  # and ("VISP_EXPORT" in stmt)):
                        decl = [stmt_type + " " + self.get_dotted_name(classname), "", modlist, [], None, docstring]
                        if bases:
                            decl[1] = ": " + ", ".join([self.get_dotted_name(b).replace(".", "::") for b in bases])
                    return stmt_type, classname, True, decl
        ...
        if (context == "struct" or context == "class") and end_token == ";" and stmt:
            ...
            if ("VP_PROP" in stmt):  # or (class_decl and ("/Map" in class_decl[2])):
                var_modlist = []
                if "VP_PROP_RW" in stmt:
                    var_modlist.append("/RW")
                stmt = self.batch_replace(stmt, [("VP_PROP_RW", ""), ("VP_PROP", "")]).strip()
                var_list = stmt.split(",")
    ...
    def parse(self, hname, wmode=True):
        ...
        for l0 in linelist:
            ...
            if l.startswith('VP__'):  # just ignore this lines
                # print('IGNORE: ' + l)
                state = SCAN
                continue
