...
public:
  ...
  vpRobotMavsdkImpl(const std::string &connection_info) : m_takeoffAlt(1.0) {
...
private:
  ...
  MAV_TYPE getVehicleType() {
    ...
    auto fut = prom.get_future();
#if (VISP_HAVE_MAVSDK_VERSION > 0x010412)
    mavsdk::MavlinkPassthrough::MessageHandle handle =
        passthrough.subscribe_message(
            MAVLINK_MSG_ID_HEARTBEAT,
            [&passthrough, &prom, &handle](const mavlink_message_t &message) {
#else
  ...
  std::function<void(mavsdk::Calibration::Result,
                     mavsdk::Calibration::ProgressData)>
...
public:
  void connect(const std::string &connectionInfo) {
    ...
    mavsdk::ConnectionResult connection_result =
        m_mavsdk.add_any_connection(connectionInfo);
    ...
    if (!m_system) {
      throw vpException(vpException::fatalError, "Unable to connect to: %s",
    ...
    std::cout << (m_has_flying_capability ? "Connected to a flying vehicle"
  ...
  void getPosition(vpHomogeneousMatrix &ned_M_frd) const {
    ...
    vpTranslationVector t{posvel.position.north_m, posvel.position.east_m,
                          posvel.position.down_m};
    ned_M_frd.buildFrom(t, q);
  }

  void getPosition(float &ned_north, float &ned_east, float &ned_down,
                   float &ned_yaw) const {
  ...
  bool sendMocapData(const vpHomogeneousMatrix &enu_M_flu, int display_fps) {
    ...
    const mavsdk::Mocap::Result set_position_result =
        mocap.set_vision_position_estimate(pose_estimate);
      ...
      if (display_fps > 0) {
        ...
        if (vpTime::measureTimeMs() - time_prev > display_time_ms) {
          ...
          std::cout << "Translation [m]: " << pose_estimate.position_body.x_m
                    << " , " << pose_estimate.position_body.y_m << " , "
  ...
  void setTakeOffAlt(double altitude) {
    if (altitude > 0) {
      ...
      std::cerr << "ERROR : The take off altitude must be positive."
                << std::endl;
    }
  }
  ...
  bool setGPSGlobalOrigin(double latitude, double longitude, double altitude) {
    ...
    mavlink_msg_set_gps_global_origin_encode(passthrough.get_our_sysid(),
                                             passthrough.get_our_compid(), &msg,
                                             &gps_global_origin);
  ...
  bool takeControl() {
    ...
    if (m_telemetry.get()->flight_mode() !=
        mavsdk::Telemetry::FlightMode::Offboard) {
    ...
    while (m_telemetry.get()->flight_mode() !=
           mavsdk::Telemetry::FlightMode::Offboard) {
  ...
  void setPositioningIncertitude(float position_incertitude,
  ...
  bool takeOff(bool interactive, int timeout_sec) {
    if (!m_has_flying_capability) {
      std::cerr << "Warning: Cannot takeoff this non flying vehicle"
                << std::endl;
      return true;
    }
    ...
    if (m_telemetry.get()->in_air()) {
      std::cerr << "Cannot take off as the robot is already flying."
                << std::endl;
      return true;
      ...
      // Takeoff
      if (m_telemetry.get()->gps_info().fix_type ==
        ...
        Telemetry::LandedStateHandle handle =
            m_telemetry.get()->subscribe_landed_state(
                [this, &in_air_promise,
...
#endif
        if (in_air_future.wait_for(seconds(timeout_sec)) ==
        ...
        auto handle_odom = m_telemetry.get()->subscribe_odometry(
            [this, &takeoff_finished_promise, &handle,
             &Z_init](mavsdk::Telemetry::Odometry odom) {
              if (odom.position_body.z_m <
                  0.90 * (Z_init - m_takeoffAlt) + m_position_incertitude) {
...
#endif
        if (takeoff_finished_future.wait_for(seconds(timeout_sec)) ==
            std::future_status::timeout) {
        ...
        std::cout << "---- DEBUG: GPS detected: use action::takeoff()"
                  << std::endl;
  ...
  bool land() {
    ...
    if (m_telemetry.get()->flight_mode() !=
        mavsdk::Telemetry::FlightMode::Land) {
      std::cout << "Landing...\n";
  ...
  bool setPosition(float ned_north, float ned_east, float ned_down,
                   float ned_yaw, bool blocking, int timeout_sec) {
    ...
    std::cout << "NED Pos to reach: " << position_target.north_m << " "
              << position_target.east_m << " " << position_target.down_m << " "
    ...
    if (m_telemetry.get()->flight_mode() !=
        mavsdk::Telemetry::FlightMode::Offboard) {
      if (m_verbose) {
        std::cout << "Cannot set vehicle position: offboard mode not started"
                  << std::endl;
      }
      return false;
    }
...
#if (VISP_HAVE_MAVSDK_VERSION > 0x010412)
      auto handle_odom = m_telemetry.get()->subscribe_odometry(
          [this, &position_reached_promise, &handle,
           &position_target](mavsdk::Telemetry::Odometry odom) {
            vpQuaternionVector q{odom.q.x, odom.q.y, odom.q.z, odom.q.w};
            vpRotationMatrix R(q);
            vpRxyzVector rxyz(R);
            ...
            if (distance_to_target < m_position_incertitude &&
                std::fabs(odom_yaw - position_target.yaw_deg) <
                    m_yaw_incertitude) {
...
#endif
      if (position_reached_future.wait_for(seconds(timeout_sec)) ==
          std::future_status::timeout) {
  ...
  bool setPositionRelative(float ned_delta_north, float ned_delta_east,
                           float ned_delta_down, float ned_delta_yaw,
    ...
    return setPosition(
        position_target.north_m, position_target.east_m, position_target.down_m,
        vpMath::rad(position_target.yaw_deg), blocking, timeout_sec);
  }

  bool setPosition(const vpHomogeneousMatrix &M, bool absolute,
                   int timeout_sec) {
    auto XYZvec = vpRxyzVector(M.getRotationMatrix());
    if (XYZvec[0] != 0.0) {
      std::cerr << "ERROR : Can't move, rotation around X axis should be 0."
                << std::endl;
      return false;
    }
    if (XYZvec[1] != 0.0) {
      std::cerr << "ERROR : Can't move, rotation around Y axis should be 0."
                << std::endl;
      return false;
    }
    return setPosition(M.getTranslationVector()[0], M.getTranslationVector()[1],
                       M.getTranslationVector()[2], XYZvec[2], absolute,
                       timeout_sec);
  }

  bool setPositionRelative(const vpHomogeneousMatrix &M, bool blocking,
                           int timeout_sec) {
    auto XYZvec = vpRxyzVector(M.getRotationMatrix());
    if (XYZvec[0] != 0.0) {
      std::cerr << "ERROR : Can't move, rotation around X axis should be 0."
    ...
    return setPositionRelative(
        M.getTranslationVector()[0], M.getTranslationVector()[1],
        M.getTranslationVector()[2], XYZvec[2], blocking, timeout_sec);
  }
  ...
  bool setVelocity(const vpColVector &frd_vel_cmd) {
    ...
    if (m_telemetry.get()->flight_mode() !=
        mavsdk::Telemetry::FlightMode::Offboard) {
      if (m_verbose) {
        std::cout << "Cannot set vehicle velocity: offboard mode not started"
                  << std::endl;
      }
      return false;
    }
    ...
    if (m_telemetry.get()->in_air()) {
      if (m_telemetry.get()->gps_info().fix_type !=
        ...
        if (m_telemetry.get()->flight_mode() !=
          ...
          if (m_verbose) {
            std::cout
                << "Cannot set vehicle velocity: offboard mode not started"
                << std::endl;
          }
          return false;
        }
  ...
  bool stopMoving() {
    if (m_telemetry.get()->flight_mode() !=
        mavsdk::Telemetry::FlightMode::Offboard) {
      if (m_verbose) {
        std::cout << "Cannot stop moving: offboard mode not started"
                  << std::endl;
      }
      return false;
    }
...
private:
  //*** Attributes ***//
  std::string
      m_address{}; ///< Ip address of the robot to discover on the network
  ...
  double m_takeoffAlt{
      1.0}; ///< The altitude to aim for when calling the function takeoff
...
vpRobotMavsdk::vpRobotMavsdk(const std::string &connection_info)
    : m_impl(new vpRobotMavsdkImpl(connection_info)) {
...
void vpRobotMavsdk::connect(const std::string &connection_info) {
  m_impl->connect(connection_info);
}
 ...
 */
void vpRobotMavsdk::getPosition(float &ned_north, float &ned_east,
                                float &ned_down, float &ned_yaw) const {
 ...
 */
bool vpRobotMavsdk::takeOff(bool interactive, int timeout_sec) {
 ...
 * \sa setTakeOffAlt(), land(), hasFlyingCapability()
 */
bool vpRobotMavsdk::takeOff(bool interactive, double takeoff_altitude,
                            int timeout_sec) {
 ...
 */
bool vpRobotMavsdk::setPosition(float ned_north, float ned_east, float ned_down,
                                float ned_yaw, bool blocking, int timeout_sec) {
  return m_impl->setPosition(ned_north, ned_east, ned_down, ned_yaw, blocking,
                             timeout_sec);
}
...
bool vpRobotMavsdk::setPositionRelative(float ned_delta_north,
                                        ...
                                        float ned_delta_yaw, bool blocking,
                                        int timeout_sec) {
  return m_impl->setPositionRelative(ned_delta_north, ned_delta_east,
                                     ned_delta_down, ned_delta_yaw, blocking,
                                     timeout_sec);
}
 ...
 * \sa setPosition(const vpHomogeneousMatrix &, bool, float)
 */
bool vpRobotMavsdk::setPositionRelative(
    const vpHomogeneousMatrix &delta_frd_M_frd, bool blocking,
    int timeout_sec) {
 ...
 */
bool vpRobotMavsdk::setGPSGlobalOrigin(double latitude, double longitude,
                                       double altitude) {
