...
namespace {
  ...
  void convert_depth_map(const vpImage<uint16_t> &depth_map_raw, vpImage<unsigned char> &depth_map) {
    vpImageConvert::convert(depth_map_raw, depth_map);

    int min = 255, max = 0;
    for (unsigned int cpt = 0; cpt < depth_map.getSize(); cpt++) {
      min = depth_map.bitmap[cpt] < min ? depth_map.bitmap[cpt] : min;
      max = depth_map.bitmap[cpt] > max ? depth_map.bitmap[cpt] : max;
    }
    ...
    if (max - min > 0) {
      ...
      for (int i = 0; i < 256; i++) {
        lut[i] = vpMath::saturate<unsigned char>( (i - min) * a );
      }

      depth_map.performLut(lut);
    }
  }
  ...
  void test_SR300(vpRealSense &rs, const std::map<rs::stream, bool> &enables, const std::map<rs::stream, vpRealSense::vpRsStreamParams> &params,
                 const std::string &title, const bool depth_color_visualization=false, const rs::stream &color_stream=rs::stream::color,
    ...
    while (true) {
      ...
      if (display_pcl) {
        ...
        if (direct_infrared_conversion) {
          rs.acquire( (unsigned char *) I_color.bitmap, (unsigned char *) depth.bitmap, NULL, (unsigned char *) I_infrared.bitmap, NULL, color_stream, depth_stream );
        } else {
          rs.acquire( (unsigned char *) I_color.bitmap, (unsigned char *) depth.bitmap, NULL, (unsigned char *) infrared.bitmap, NULL, color_stream, depth_stream );
          vpImageConvert::convert(infrared, I_infrared);
        }
      }
      ...
      if (depth_color_visualization) {
        ...
        convert_depth_map(depth, I_depth);
      }
...
int main(int argc, char *argv[]) {
  try {
    ...
    test_SR300(rs, enables, params, "SR300_COLOR_ALIGNED_TO_DEPTH_RGBA8_640x480_30FPS + SR300_DEPTH_Z16_640x480_30FPS", true, rs::stream::color_aligned_to_depth);


    //Depth stream aligned to color
    test_SR300(rs, enables, params, "SR300_COLOR_RGBA8_640x480_30FPS + SR300_DEPTH_ALIGNED_TO_COLOR_Z16_640x480_30FPS", true, rs::stream::color, rs::stream::depth_aligned_to_color);
