...
vpRealSense2::vpRealSense2()
  : m_colorIntrinsics(), m_depth2ColorExtrinsics(), m_depthIntrinsics(), m_depthScale(0.0f),
    m_invalidDepthValue(0.0f), m_max_Z(8.0f), m_pipe(), m_pipelineProfile(), m_pointcloud(),
    m_points() {
}
 ...
 */
void vpRealSense2::acquire(unsigned char * const data_image, unsigned char * const data_depth, std::vector<vpColVector> * const data_pointCloud,
                           unsigned char * const data_infrared, rs2::align * const align_to) {
  auto data = m_pipe.wait_for_frames();
  if (align_to != NULL)
 ...
 */
void vpRealSense2::acquire(unsigned char * const data_image, unsigned char * const data_depth, std::vector<vpColVector> * const data_pointCloud,
                           pcl::PointCloud<pcl::PointXYZ>::Ptr &pointcloud, unsigned char * const data_infrared, rs2::align * const align_to) {
  auto data = m_pipe.wait_for_frames();
  if (align_to != NULL)
    data = align_to->proccess(data);
 ...
 */
void vpRealSense2::acquire(unsigned char * const data_image, unsigned char * const data_depth, std::vector<vpColVector> * const data_pointCloud,
                           pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pointcloud, unsigned char * const data_infrared, rs2::align * const align_to) {
  auto data = m_pipe.wait_for_frames();
  if (align_to != NULL)
    data = align_to->proccess(data);
...
/*!
   ...
   \sa getIntrinsics()
 */
vpCameraParameters vpRealSense2::getCameraParameters(const rs2_stream &stream, vpCameraParameters::vpCameraParametersProjType type) const {
  ...
  for (int i = 0; i < height; i++) {
    ...
    for (int j = 0; j < width; j++, depth_pixel_index++) {
      ...
      else
      {
        ...
        uint32_t rgb = 0;
        if (swap_rgb) {
          rgb = (static_cast<uint32_t>(p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel]) |
...
#else
        if (swap_rgb) {
          pointcloud->points[(size_t) depth_pixel_index].b = (uint32_t) p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel];
          pointcloud->points[(size_t) depth_pixel_index].g = (uint32_t) p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel + 1];
          pointcloud->points[(size_t) depth_pixel_index].r = (uint32_t) p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel + 2];
        } else {
          pointcloud->points[(size_t) depth_pixel_index].r = (uint32_t) p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel];
          pointcloud->points[(size_t) depth_pixel_index].g = (uint32_t) p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel + 1];
          pointcloud->points[(size_t) depth_pixel_index].b = (uint32_t) p_color_frame[(i_* (unsigned int) color_width + j_) * nb_color_pixel + 2];
        }
#endif
      }
    }
...
void print(const rs2_intrinsics& intrinsics, std::ostream &os) {
  std::stringstream ss;
  ss << std::left << std::setw(14) << "Width: "      << "\t" << intrinsics.width  << "\n" <<
...
std::ostream & operator<<(std::ostream &os, const vpRealSense2 &rs) {
  os << std::left << std::setw(30) << "Device Name"
     << std::setw(20) << "Serial Number"
     << std::setw(20) << "Firmware Version"
     << std::endl;
  ...
  for (auto&& sensor : dev.query_sensors()) {
    ...
    os << std::setw(55) << " Supported options:" << std::setw(10) << "min" << std::setw(10)
       << " max" << std::setw(6) << " step" << std::setw(10) << " default" << std::endl;
    for (auto j = 0; j < RS2_OPTION_COUNT; ++j) {
      ...
      if (sensor.supports(opt)) {
        ...
        os << "    " << std::left << std::setw(50) << opt << " : "
           << std::setw(5) << range.min << "... " << std::setw(12) << range.max
           << std::setw(6) << range.step << std::setw(10) << range.def << "\n";
      }
    }
  ...
  for (auto&& sensor : dev.query_sensors()) {
    ...
    os << std::setw(55) << " Supported modes:" << std::setw(10) << "stream" << std::setw(10)
       << " resolution" << std::setw(6) << " fps" << std::setw(10) << " format" << "\n";
    // Show which streams are supported by this device
    for (auto&& profile : sensor.get_stream_profiles()) {
      if (auto video = profile.as<rs2::video_stream_profile>()) {
        os << "    " << profile.stream_name() << "\t  " << video.width() << "x"
           << video.height() << "\t@ " << profile.fps() << "Hz\t" << profile.format() << "\n";
      } else {
  ...
  for (auto& kvp : intrinsics_map) {
    ...
    for (auto& intrinsics : kvp.second) {
      ...
      os << "Intrinsic of \"" << stream_res.stream_name << "\"\t  " << stream_res.width << "x"
         << stream_res.height << "\t  " << formats << "\n";
...
// Work arround to avoid warning: libvisp_sensor.a(vpRealSense2.cpp.o) has no symbols
