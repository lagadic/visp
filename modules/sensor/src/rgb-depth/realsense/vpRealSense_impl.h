...
template <class Type>
void vp_rs_get_frame_data_impl(
    const rs::device *m_device,
    ...
    const rs::stream &stream, vpImage<Type> &data)
{
  ...
  if (m_device->is_stream_enabled(stream)) {
    std::map<rs::stream, rs::intrinsics>::const_iterator it_intrinsics =
    ...
    memcpy((unsigned char *)data.bitmap,
           (unsigned char *)m_device->get_frame_data(stream),
           width * height * sizeof(Type));
  } else {
    throw vpException(vpException::fatalError,
                      "RealSense Camera - stream %d not enabled!",
                      (rs_stream)stream);
  }
}

// Retrieve data from native stream
void vp_rs_get_native_frame_data_impl(
    const rs::device *m_device,
    const std::map<rs::stream, rs::intrinsics> &m_intrinsics,
    ...
    const rs::stream &stream)
{
  if (m_device->is_stream_enabled(native_stream)) {
    std::map<rs::stream, rs::intrinsics>::const_iterator it_intrinsics =
        m_intrinsics.find(native_stream);
    if (it_intrinsics == m_intrinsics.end()) {
      std::stringstream ss;
    ...
    size_t size =
        (size_t)(it_intrinsics->second.width * it_intrinsics->second.height);
    ...
    case rs::format::raw10:
      ...
      memcpy(data, (unsigned char *)m_device->get_frame_data(stream),
             (5 * (size / 4)));
      break;
    ...
    throw vpException(vpException::fatalError,
                      "RealSense Camera - stream %d not enabled!",
                      (rs_stream)native_stream);
  }
}

// Retrieve color image
void vp_rs_get_color_impl(
    const rs::device *m_device,
    const std::map<rs::stream, rs::intrinsics> &m_intrinsics,
    vpImage<vpRGBa> &color)
{
  if (m_device->is_stream_enabled(rs::stream::color)) {
    std::map<rs::stream, rs::intrinsics>::const_iterator it_intrinsics =
        m_intrinsics.find(rs::stream::color);
    if (it_intrinsics == m_intrinsics.end()) {
      throw vpException(vpException::fatalError,
                        "Cannot find intrinsics for color stream!");
    }
    ...
    if (m_device->get_stream_format(rs::stream::color) == rs::format::rgb8) {
      vpImageConvert::RGBToRGBa(
          (unsigned char *)m_device->get_frame_data(rs::stream::color),
          (unsigned char *)color.bitmap, width, height);
    } else if (m_device->get_stream_format(rs::stream::color) ==
               rs::format::rgba8) {
      memcpy((unsigned char *)color.bitmap,
             (unsigned char *)m_device->get_frame_data(rs::stream::color),
    ...
    throw vpException(vpException::fatalError,
                      "RealSense Camera - color stream not enabled!");
  }
}

// Retrieve grey image
void vp_rs_get_grey_impl(
    const rs::device *m_device,
    const std::map<rs::stream, rs::intrinsics> &m_intrinsics,
    vpImage<unsigned char> &grey)
{
  if (m_device->is_stream_enabled(rs::stream::color)) {
    ...
    if (m_device->get_stream_format(rs::stream::color) == rs::format::rgb8) {
      vpImageConvert::RGBToGrey(
          (unsigned char *)m_device->get_frame_data(rs::stream::color),
          (unsigned char *)grey.bitmap, width, height);
    } else if (m_device->get_stream_format(rs::stream::color) ==
               rs::format::rgba8) {
      vpImageConvert::RGBaToGrey(
          (unsigned char *)m_device->get_frame_data(rs::stream::color),
          (unsigned char *)grey.bitmap, width * height);
    } else if (m_device->get_stream_format(rs::stream::color) ==
               rs::format::bgr8) {
...
void vp_rs_get_pointcloud_impl(
    const rs::device *m_device,
    const std::map<rs::stream, rs::intrinsics> &m_intrinsics, float max_Z,
    std::vector<vpColVector> &pointcloud,
    const float invalidDepthValue = 0.0f,
    const rs::stream &stream_depth = rs::stream::depth)
{
  if (m_device->is_stream_enabled(rs::stream::depth)) {
    std::map<rs::stream, rs::intrinsics>::const_iterator it_intrinsics =
        m_intrinsics.find(stream_depth);
    if (it_intrinsics == m_intrinsics.end()) {
      throw vpException(vpException::fatalError,
                        "Cannot find intrinsics for depth stream!");
    }
    ...
    for (int i = 0; i < height; i++) {
      for (int j = 0; j < width; j++) {
        ...
        depth_point =
            it_intrinsics->second.deproject(depth_pixel, scaled_depth);
...
{
  if (m_device->is_stream_enabled(rs::stream::depth) &&
      m_device->is_stream_enabled(rs::stream::color)) {
    std::map<rs::stream, rs::intrinsics>::const_iterator it_intrinsics_depth =
        m_intrinsics.find(stream_depth);
    ...
    std::map<rs::stream, rs::intrinsics>::const_iterator it_intrinsics_color =
        m_intrinsics.find(stream_color);
    ...
    if (m_device->get_stream_format(rs::stream::color) != rs::format::rgb8 &&
      ...
      throw vpException(
          ...
          "Color stream type must be rgb8, rgba8, bgr8 or bgra8!");
    }
    ...
    const float depth_scale = m_device->get_depth_scale();
    rs::extrinsics depth_2_color_extrinsic =
        m_device->get_extrinsics(stream_depth, stream_color);

    const uint16_t *depth =
        (uint16_t *)m_device->get_frame_data(stream_depth);
    const unsigned char *color =
        (unsigned char *)m_device->get_frame_data(stream_color);
    ...
    unsigned int nb_color_pixel =
        (m_device->get_stream_format(rs::stream::color) == rs::format::rgb8 ||
         m_device->get_stream_format(rs::stream::color) == rs::format::bgr8)
            ? 3
            : 4;
    ...
    for (int index = 0; index < nb_threads; index++) {
      ...
      int end_index =
          index == nb_threads - 1 ? depth_height : (index + 1) * step;
      ...
      workers.push_back(std::thread([&, start_index, end_index]() {
        for (int i = start_index; i < end_index; i++) {
          for (int j = 0; j < depth_width; j++) {
            ...
            rs::float2 depth_pixel = {(float)j, (float)i};
            rs::float3 depth_point = it_intrinsics_depth->second.deproject(
            ...
            pointcloud->points[(size_t)(i * depth_width + j)].x =
                depth_point.x;
            pointcloud->points[(size_t)(i * depth_width + j)].y =
                depth_point.y;
            pointcloud->points[(size_t)(i * depth_width + j)].z =
                depth_point.z;

            rs::float3 color_point =
                depth_2_color_extrinsic.transform(depth_point);
            rs::float2 color_pixel =
                it_intrinsics_color->second.project(color_point);
...
// out of bounds color value.
              ...
              if (swap_rgb) {
                rgb = (static_cast<uint32_t>(
                           color[(i_ * (unsigned int)color_width + j_) *
                                 nb_color_pixel]) |
              ...
              if (swap_rgb) {
                pointcloud->points[(size_t)(i * depth_width + j)].b =
                    (uint32_t)color[(i_ * (unsigned int)color_width + j_) *
                                    nb_color_pixel];
                pointcloud->points[(size_t)(i * depth_width + j)].g =
                    (uint32_t)color[(i_ * (unsigned int)color_width + j_) *
                                        nb_color_pixel +
