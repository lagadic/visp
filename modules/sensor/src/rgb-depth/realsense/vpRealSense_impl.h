                               ...
                               const rs::stream &stream, vpImage<Type> &data)
{
  ...
  if (m_device->is_stream_enabled(stream)) {
    ...
    unsigned int width = static_cast<unsigned int>(it_intrinsics->second.width);
    unsigned int height = static_cast<unsigned int>(it_intrinsics->second.height);
    data.resize(height, width);

    memcpy(reinterpret_cast<unsigned char *>(data.bitmap), reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)),
                                      ...
                                      const rs::stream &stream)
{
  if (m_device->is_stream_enabled(native_stream)) {
    ...
    size_t size = static_cast<size_t>(it_intrinsics->second.width * it_intrinsics->second.height);
    ...
    case rs::format::raw8:
      ...
      /* FALLTHRU */
    case rs::format::y8:
      memcpy(data, reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)), size);
      break;
    ...
    case rs::format::raw10:
      ...
      memcpy(data, reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)), (5 * (size / 4)));
      break;

    // 16 bits
    case rs::format::raw16:
      std::cout << "Stream: raw16 not tested!" << std::endl;
      memcpy(data, reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)), size * 2);
      break;
    ...
    case rs::format::bgr8:
    case rs::format::rgb8:
      memcpy(data, reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)), size * 3);
      break;

    // 32 bits
    case rs::format::yuyv:
      std::cout << "Stream: yuyv not tested!" << std::endl;
      memcpy(data, reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)), size * 4);
      break;
    ...
    case rs::format::xyz32f:
      ...
      memcpy(data, reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream)), size * 3 * 4);
      break;
                          ...
                          vpImage<vpRGBa> &color)
{
  if (m_device->is_stream_enabled(rs::stream::color)) {
    ...
    if (m_device->get_stream_format(rs::stream::color) == rs::format::rgb8) {
      vpImageConvert::RGBToRGBa(reinterpret_cast<unsigned char *>(m_device->get_frame_data(rs::stream::color)),
      ...
      memcpy(reinterpret_cast<unsigned char *>(color.bitmap), reinterpret_cast<unsigned char *>(m_device->get_frame_data(rs::stream::color)),
             width * height * sizeof(vpRGBa));
    }
    else if (m_device->get_stream_format(rs::stream::color) == rs::format::bgr8) {
      vpImageConvert::BGRToRGBa(reinterpret_cast<unsigned char *>(m_device->get_frame_data(rs::stream::color)),
                                reinterpret_cast<unsigned char *>(color.bitmap), width, height);
    }
    else {
                         ...
                         vpImage<unsigned char> &grey)
{
  if (m_device->is_stream_enabled(rs::stream::color)) {
    ...
    if (m_device->get_stream_format(rs::stream::color) == rs::format::rgb8) {
      vpImageConvert::RGBToGrey(reinterpret_cast<unsigned char *>(m_device->get_frame_data(rs::stream::color)),
                                reinterpret_cast<unsigned char *>(grey.bitmap), width, height);
    }
    else if (m_device->get_stream_format(rs::stream::color) == rs::format::rgba8) {
      vpImageConvert::RGBaToGrey(reinterpret_cast<unsigned char *>(m_device->get_frame_data(rs::stream::color)),
                                 reinterpret_cast<unsigned char *>(grey.bitmap), width * height);
    }
    else if (m_device->get_stream_format(rs::stream::color) == rs::format::bgr8) {
      vpImageConvert::BGRToGrey(reinterpret_cast<unsigned char *>(m_device->get_frame_data(rs::stream::color)),
                                reinterpret_cast<unsigned char *>(grey.bitmap), width, height);
    }
    else {
      throw vpException(vpException::fatalError, "RealSense Camera - color stream not supported!");
                               ...
                               const rs::stream &stream_depth = rs::stream::depth)
{
  if (m_device->is_stream_enabled(rs::stream::depth)) {
    ...
    uint16_t *depth = reinterpret_cast<uint16_t *>(m_device->get_frame_data(stream_depth));
    ...
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        ...
        rs::float2 depth_pixel = { static_cast<float>j, static_cast<float>i };
        ...
        pointcloud[static_cast<size_t>(i * width + j)] = p3d;
      }
    }
  }
  else {
  ...
  if (m_device->is_stream_enabled(rs::stream::depth)) {
    ...
    for (int i = 0; i < height; ++i) {
      for (int j = 0; j < width; ++j) {
        ...
        pointcloud->points[static_cast<size_t>(i * width + j)].x = depth_point.x;
        pointcloud->points[static_cast<size_t>(i * width + j)].y = depth_point.y;
        pointcloud->points[static_cast<size_t>(i * width + j)].z = depth_point.z;
      }
    }
  }
  else {
                               ...
                               const rs::stream &stream_depth = rs::stream::depth)
{
  if (m_device->is_stream_enabled(rs::stream::depth) && m_device->is_stream_enabled(rs::stream::color)) {
    ...
    pointcloud->resize(static_cast<size_t>(depth_width * depth_height));

    const float depth_scale = m_device->get_depth_scale();
    rs::extrinsics depth_2_color_extrinsic = m_device->get_extrinsics(stream_depth, stream_color);

    const uint16_t *depth = reinterpret_cast<uint16_t *>(m_device->get_frame_data(stream_depth));
    const unsigned char *color = reinterpret_cast<unsigned char *>(m_device->get_frame_data(stream_color));
      ...
      workers.push_back(std::thread([&, start_index, end_index]() {
        for (int i = start_index; i < end_index; ++i) {
          for (int j = 0; j < depth_width; ++j) {
            ...
            pointcloud->points[static_cast<size_t>(i * depth_width + j)].x = depth_point.x;
            pointcloud->points[static_cast<size_t>(i * depth_width + j)].y = depth_point.y;
            pointcloud->points[static_cast<size_t>(i * depth_width + j)].z = depth_point.z;
...
#else
              pointcloud->points[static_cast<size_t>(i * depth_width + j)].r = static_cast<uint8_t>(96);
              pointcloud->points[static_cast<size_t>(i * depth_width + j)].g = static_cast<uint8_t>(157);
              pointcloud->points[static_cast<size_t>(i * depth_width + j)].b = static_cast<uint8_t>(198);
#endif
            }
            else {
