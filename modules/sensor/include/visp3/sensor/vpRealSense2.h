...
/*!
  ...
  This class provides a lightweight wrapper over the Intel librealsense2 library https://github.com/IntelRealSense/librealsense.
  ...
  a CMakeLists.txt file that allows to build sample-realsense.cpp that uses vpRealSense2 class.
  \code
project(sample)
...
int main()
{
  ...
  If you want to change the default stream parameters, refer to the librealsense2 `rs2::config` documentation.
  The following code allows to capture the color stream in 1920x1080:
  \code
#include <visp3/sensor/vpRealSense2.h>
#include <visp3/gui/vpDisplayGDI.h>
#include <visp3/gui/vpDisplayX.h>
...
int main() {
  ...
  vpImage<unsigned char> Ii(rs.getIntrinsics(RS2_STREAM_INFRARED).height, rs.getIntrinsics(RS2_STREAM_INFRARED).width);
...
int main() {
  ...
  \note This class has been tested with the Intel RealSense SR300 (Firmware: 3.21.0.0)
  using librealsense (API version: 2.8.3).
  Refer to the librealsense2 documentation or [API how to](https://github.com/IntelRealSense/librealsense/wiki/API-How-To)
...
public:
...
#ifdef VISP_HAVE_PCL
  void acquire(unsigned char * const data_image, unsigned char * const data_depth, std::vector<vpColVector> * const data_pointCloud, pcl::PointCloud<pcl::PointXYZ>::Ptr &pointcloud,
               unsigned char * const data_infrared=NULL, rs2::align * const align_to=NULL);
  void acquire(unsigned char * const data_image, unsigned char * const data_depth, std::vector<vpColVector> * const data_pointCloud, pcl::PointCloud<pcl::PointXYZRGB>::Ptr &pointcloud,
               unsigned char * const data_infrared=NULL, rs2::align * const align_to=NULL);
#endif
  ...
  inline void setMaxZ(const float maxZ) {
    m_max_Z = maxZ;
  }
