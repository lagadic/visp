...
/*!
  ...
  This class is a wrapper over six axis load measurement (forces and torques) from IIT (ISTITUTO ITALIANO DI TECNOLOGIA) sell
  by [Alberobotics](https://alberobotics.it/sensors-data-sheet/file).
...
LOCAL_IFACE = enp0s31f6                ; Host Computer's local interface name (it is required only in Linux)
  \endverbatim
  - The sensor default IP is set to `192.168.1.1`. Browsing to this address with Firefox allows to modify it's default IP address.
  If the new sensor IP address is for example `192.168.100.10`, you need to modify the following line:
  \verbatim
...
./testForceTorqueIitSensor-simple: error while loading shared libraries: libftSensorLib.so.0.0.1: cannot open shared object file: No such file or directory
  \endcode
  ...
  \code
$ export LD_LIBRARY_PATH=$LD_LIBRARY_PATH:$VISP_WS/3rdparty/FT_SDK_01_4\(June2019\)/linux/ubuntu16.04/ftSensorLibReleaseExamples/library/bin/lin-x86_64/release
  \endcode
