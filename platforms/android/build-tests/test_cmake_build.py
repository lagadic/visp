...
class TestCmakeBuild(unittest.TestCase):
    def __init__(self, libset, abi, toolchain, visp_cmake_path, workdir, *args, **kwargs):
        unittest.TestCase.__init__(self, *args, **kwargs)
        self.libset = libset
        self.abi = abi
        self.toolchain = toolchain
        self.visp_cmake_path = visp_cmake_path
    ...
    def runTest(self):
        cmd = [
            "cmake",
            "-GNinja",
            "-DVISP_DIR=%s" % self.visp_cmake_path,
            "-DANDROID_ABI=%s" % self.abi,
            "-DCMAKE_TOOLCHAIN_FILE=%s" % os.path.join(self.visp_cmake_path, "android.toolchain.cmake"),
...
def suite(workdir, visp_cmake_path):
    abis = {
    ...
    for libset in ["", "opencv_java"]:
        for abi, toolchain in abis.items():
            suite.addTest(TestCmakeBuild(libset, abi, toolchain, visp_cmake_path, workdir))
    return suite


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test VISP for Android SDK with cmake')
    parser.add_argument('--sdk_path', help="Path to Android SDK to use for build")
    parser.add_argument('--ndk_path', help="Path to Android NDK to use for build")
    parser.add_argument("--workdir", default="testspace", help="Working directory (and output)")
    parser.add_argument("visp_cmake_path", help="Path to folder with VISPConfig.cmake and android.toolchain.cmake (usually <SDK>/sdk/native/jni/")
    ...
    res = unittest.TextTestRunner(verbosity=3).run(suite(os.path.abspath(args.workdir), os.path.abspath(args.visp_cmake_path)))
