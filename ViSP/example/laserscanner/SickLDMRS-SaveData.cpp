...
/*!
  \example SickLDMRS-SaveData.cpp
  ...
  laser scans and how to save the scan data in a file.  
...
int main(int argc, const char ** argv)
{
  ...
  while(1) {
	...
	continue;
      ...
      // Set the data filename to store the measures
      sprintf(filename, "scan%d-layer%d.txt", iter, layer+1);
      std::ofstream fd(filename);

      // Write the file header
      fd << "# Scan layer [1 to 4] : " << layer+1 << std::endl
	 << "# Start timestamp     : " << laserscan[layer].getStartTimestamp() 
	 << std::endl
	 << "# End timestamp       : " << laserscan[layer].getEndTimestamp() 
	 << std::endl
