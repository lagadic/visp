...
int main()
{
  ...
  kalman.initFilter(signal, sigma_state, sigma_measure, rho);
}
  \endcode
...
int main()
{
  ...
  kalman.initFilter(nsignal, sigma_state, sigma_measure, rho);
 ...
 */
void
vpKalmanFilter::initFilter(int nsignal, 
...
int main()
{
  ...
  // Initialize the filter
  kalman.initFilterWithColoredNoise(nsignal, sigma_state, sigma_measure, rho);
...
*/
void 
vpKalmanFilter::initStateConstVelWithColoredNoise_MeasureVel(int nsignal,
					       vpColVector &sigma_state,
					       vpColVector &sigma_measure,
					       double rho)
{
...
/*!
  ...
  \code
#include <visp/vpKalmanConstantAcceleration.h>

int main()
{
  vpKalmanConstantAcceleration kalman;
  // Filter the x and y velocities of a target (2 signals are to consider)
  int nsignal = 2;
  ...
  // Initialize the filter
  kalman.initFilterWithColoredNoise(nsignal, sigma_state, sigma_measure, rho, dt);
...
*/
void
vpKalmanFilter::initStateConstAccWithColoredNoise_MeasureVel(int nsignal, 
							 vpColVector &sigma_state,
							 vpColVector &sigma_measure,
							 double rho,
							 double dt)
