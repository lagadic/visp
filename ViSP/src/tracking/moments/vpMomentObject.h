...
int main()
{
  ...
  vpMomentObject obj(4); // use moments up to order 4
  ...
  for(int j=0; j<obj.getOrder(); ++j) {
    for(int i=0; i<obj.getOrder(); ++i)
      std::cout << "m" << i << j << "=" << moment[j*(obj.getOrder()+1) + i] << "  ";
    std::cout << std::endl;
  }
