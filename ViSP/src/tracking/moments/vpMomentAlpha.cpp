...
*/
vpMomentAlpha::vpMomentAlpha() : vpMoment(),isRef(true){
    values.resize(1);
}
...
*/
vpMomentAlpha::vpMomentAlpha(std::vector<double>& ref,double alphaRef) : vpMoment(),isRef(false),ref(ref),alphaRef(alphaRef){
    values.resize(1);
}
...
void vpMomentAlpha::compute(){
    ...
    if(isRef){
	  ...
	  if(std::abs(rotMu[i])>1e10 * std::numeric_limits<double>::epsilon() && std::abs(ref[i])>1e10 * std::numeric_limits<double>::epsilon() && rotMu[i]*ref[i]>0) signChange=false;
	  sum+=std::abs(rotMu[i]*ref[i]);
        }

        if(sum<1e4 * std::numeric_limits<double>::epsilon()) signChange=false;
        if(signChange)
            alpha=alpha+3.14;
