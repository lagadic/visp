...
bool getOptions(int argc, const char **argv, std::string &ipath,
                std::string &configFile, std::string &configFile_depth,
                std::string &modelFile, std::string &modelFile_depth,
                std::string &initFile, bool &displayFeatures,
                bool &click_allowed, bool &display, bool &useOgre,
                bool &showOgreConfigDialog, bool &useScanline,
                bool &computeCovariance, bool &projectionError,
                int &trackerType, int &tracker_type_depth, int &lastFrame)
{
  const char *optarg_;
  int c;
...
void rs_deproject_pixel_to_point(float point[3], const rs_intrinsics &intrin,
                                 const float pixel[2], float depth)
{
  ...
  float ux = x * f + 2 * intrin.coeffs[2] * x * y +
             intrin.coeffs[3] * (r2 + 2 * x * x);
  float uy = y * f + 2 * intrin.coeffs[3] * x * y +
             intrin.coeffs[2] * (r2 + 2 * y * y);
...
bool read_data(const unsigned int cpt, const std::string &input_directory,
               ...
               unsigned int &pointcloud_height)
{
  ...
  std::ifstream file_depth(filename_depth.c_str(),
  ...
  for (unsigned int i = 0; i < height; i++) {
    for (unsigned int j = 0; j < width; j++) {
      ...
      rs_deproject_pixel_to_point(point, depth_intrinsic, pixel,
                                  scaled_depth);
...
// Klt
  ...
  // Depth
  dynamic_cast<vpMbGenericTracker *>(tracker)
      ->setDepthNormalFeatureEstimationMethod(
          vpMbtFaceDepthNormal::ROBUST_FEATURE_ESTIMATION);
  dynamic_cast<vpMbGenericTracker *>(tracker)
      ->setDepthNormalPclPlaneEstimationMethod(2);
  dynamic_cast<vpMbGenericTracker *>(tracker)
      ->setDepthNormalPclPlaneEstimationRansacMaxIter(200);
  dynamic_cast<vpMbGenericTracker *>(tracker)
      ->setDepthNormalPclPlaneEstimationRansacThreshold(0.001);
  dynamic_cast<vpMbGenericTracker *>(tracker)->setDepthNormalSamplingStep(2,
                                                                          2);

  dynamic_cast<vpMbGenericTracker *>(tracker)->setDepthDenseSamplingStep(4,
                                                                         4);

  vpCameraParameters cam1, cam2;
  cam1.initPersProjWithoutDistortion(615.1674804688, 615.1675415039,
                                     312.1889953613, 243.4373779297);
  cam2.initPersProjWithoutDistortion(476.0536193848, 476.0534973145,
                                     311.4845581055, 246.2832336426);

  dynamic_cast<vpMbGenericTracker *>(tracker)->setCameraParameters(cam1,
                                                                   cam2);
...
{
  {
    // Test TukeyEstimator
    {
      ...
      for (size_t i = 0; i < weights.size(); i++) {
        std::cout << "residues[" << i << "]=" << residues[i] << " ; weights[i"
                  << i << "]=" << weights[i] << std::endl;
      }
      std::cout << std::endl;
    }
  ...
  try {
    ...
    if (!getOptions(argc, argv, opt_ipath, opt_configFile,
                    opt_configFile_depth, opt_modelFile, opt_modelFile_depth,
                    opt_initFile, displayFeatures, opt_click_allowed,
                    opt_display, useOgre, showOgreConfigDialog, useScanline,
                    computeCovariance, projectionError, trackerType_image,
    ...
    ipath =
        vpIoTools::createFilePath(!opt_ipath.empty() ? opt_ipath : env_ipath,
                                  "mbt-depth/castel/castel");

    std::string dir_path = vpIoTools::createFilePath(
        !opt_ipath.empty() ? opt_ipath : env_ipath, "mbt-depth");
    if (!vpIoTools::checkDirectory(dir_path)) {
      std::cerr << "ViSP-images does not contain the folder: " << dir_path
                << "!" << std::endl;
      return EXIT_SUCCESS;
    }
    ...
    if (!opt_configFile.empty())
      ...
      configFile = vpIoTools::createFilePath(!opt_ipath.empty() ? opt_ipath
                                                                : env_ipath,
                                             "mbt-depth/castel/chateau.xml");

    if (!opt_configFile_depth.empty())
      configFile_depth = opt_configFile_depth;
    else
      configFile_depth = vpIoTools::createFilePath(
          !opt_ipath.empty() ? opt_ipath : env_ipath,
          "mbt-depth/castel/chateau_depth.xml");
    ...
    (COIN_MAJOR_VERSION == 2 || COIN_MAJOR_VERSION == 3 ||                   \
     COIN_MAJOR_VERSION == 4)
      modelFile = vpIoTools::createFilePath(
          !opt_ipath.empty() ? opt_ipath : env_ipath,
          "mbt-depth/castel/chateau_gantry.wrl");
#else
      modelFile = vpIoTools::createFilePath(!opt_ipath.empty() ? opt_ipath
                                                               : env_ipath,
                                            "mbt-depth/castel/chateau.cao");
#endif
    }
    ...
    if (!opt_modelFile_depth.empty())
      ...
      modelFile_depth = vpIoTools::createFilePath(
          !opt_ipath.empty() ? opt_ipath : env_ipath,
          "mbt-depth/castel/chateau.cao");

    std::string vrml_ext = ".wrl";
    bool use_vrml =
        (modelFile.compare(modelFile.length() - vrml_ext.length(),
                           vrml_ext.length(), vrml_ext) == 0) ||
        (modelFile_depth.compare(modelFile_depth.length() - vrml_ext.length(),
                                 vrml_ext.length(), vrml_ext) == 0);
    ...
    if (!opt_initFile.empty())
      ...
      initFile = vpIoTools::createFilePath(!opt_ipath.empty() ? opt_ipath
                                                              : env_ipath,
                                           "mbt-depth/castel/chateau.init");

    vpImage<unsigned char> I, I_depth;
    vpImage<uint16_t> I_depth_raw;
    std::vector<vpColVector> pointcloud;
    unsigned int pointcloud_width, pointcloud_height;
    if (!read_data(0, ipath, I, I_depth_raw, pointcloud, pointcloud_width,
                   pointcloud_height)) {
...
// initialise a  display
    ...
    if (opt_display) {
      ...
      display2.init(I_depth,
                    (int)(I.getWidth() / vpDisplay::getDownScalingFactor(I)) +
                        110,
    ...
    std::string depth_M_color_filename =
        vpIoTools::createFilePath(!opt_ipath.empty() ? opt_ipath : env_ipath,
                                  "mbt-depth/castel/depth_M_color.txt");
    {
      ...
      std::map<std::string, vpHomogeneousMatrix>
          mapOfCameraTransformationMatrices;
      mapOfCameraTransformationMatrices["Camera2"] = depth_M_color;
      dynamic_cast<vpMbGenericTracker *>(tracker)
          ->setCameraTransformationMatrix(mapOfCameraTransformationMatrices);
    }
    ...
    dynamic_cast<vpMbGenericTracker *>(tracker)->getCameraParameters(cam1,
                                                                     cam2);
    ...
    if (opt_display && opt_click_allowed) {
      while (!vpDisplay::getClick(I, false)) {
        ...
        vpDisplay::displayText(
            I, 15, 10, "click after positioning the object", vpColor::red);
        vpDisplay::flush(I);
      }
    }

    // Load the 3D model (either a vrml file or a .cao file)
    dynamic_cast<vpMbGenericTracker *>(tracker)->loadModel(modelFile,
                                                           modelFile_depth);
    ...
    if (opt_display && opt_click_allowed) {
      ...
      dynamic_cast<vpMbGenericTracker *>(tracker)->display(
          I, I_depth, c1Mo, c2Mo, cam1, cam2, vpColor::red);
    } else {
      vpHomogeneousMatrix c1Moi(0.06846423368, 0.09062570884, 0.3401096693,
                                -2.671882598, 0.1174275908, -0.6011935263);
      vpHomogeneousMatrix c2Moi(0.04431452054, 0.09294637757, 0.3357760654,
                                -2.677922443, 0.121297639, -0.6028463357);
      dynamic_cast<vpMbGenericTracker *>(tracker)->initFromPose(I, I_depth,
                                                                c1Moi, c2Moi);
    }
    ...
    // track the model
    {
      std::map<std::string, const vpImage<unsigned char> *> mapOfImages;
      mapOfImages["Camera1"] = &I;
      std::map<std::string, const std::vector<vpColVector> *>
          mapOfPointclouds;
      ...
      dynamic_cast<vpMbGenericTracker *>(tracker)->track(
          mapOfImages, mapOfPointclouds, mapOfWidths, mapOfHeights);
    }
    dynamic_cast<vpMbGenericTracker *>(tracker)->getPose(c1Mo, c2Mo);
    ...
    while (read_data(frame_index, ipath, I, I_depth_raw, pointcloud,
      ...
      if (frame_index == 10) {
        ...
        loadConfiguration(tracker, configFile, configFile_depth);
        dynamic_cast<vpMbGenericTracker *>(tracker)->loadModel(
            modelFile, modelFile_depth);
...
// Test to set an initial pose
      ...
      if (frame_index == 20) {
        c1Mo.buildFrom(0.07734634051, 0.08993639906, 0.342344402,
                       -2.708409543, 0.0669276477, -0.3798958303);
        c2Mo.buildFrom(0.05319520317, 0.09223511976, 0.3380095812,
                       -2.71438192, 0.07141055397, -0.3810081638);
#else
      if (frame_index == 50) {
        c1Mo.buildFrom(0.09280663035, 0.09277655672, 0.330415149,
                       -2.724431817, 0.0293932671, 0.02027966377);
        c2Mo.buildFrom(0.06865933578, 0.09494713501, 0.3260555142,
                       -2.730027451, 0.03498390135, 0.01989831338);
#endif
        std::cout << "Test set pose" << std::endl;
        dynamic_cast<vpMbGenericTracker *>(tracker)->setPose(I, I_depth, c1Mo,
                                                             c2Mo);
      }
      ...
      if (frame_index < 30 || frame_index >= 50) {
        ...
        if (opt_display) {
          ...
          dynamic_cast<vpMbGenericTracker *>(tracker)->display(
              I, I_depth, c1Mo, c2Mo, cam1, cam2, vpColor::darkRed);
          // display the frame
    ...
    std::cout << "\nComputation time, Mean: " << vpMath::getMean(time_vec)
              << " ms ; Median: " << vpMath::getMedian(time_vec)
              << " ms ; Std: " << vpMath::getStdev(time_vec) << " ms"
              << std::endl;
