...
vpThread::Return displayPointcloudFunction(vpThread::Args args)
{
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr pointcloud_ =
      *((pcl::PointCloud<pcl::PointXYZRGB>::Ptr *)args);
  ...
  do {
    ...
    if (capture_state_ == capture_started) {
      ...
      if (!update) {
        viewer->addPointCloud<pcl::PointXYZRGB>(pointcloud_, rgb,
                                                "sample cloud");
        viewer->setPointCloudRenderingProperties(
            pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "sample cloud");
        update = true;
      } else {
        viewer->updatePointCloud<pcl::PointXYZRGB>(pointcloud_, rgb,
                                                   "sample cloud");
      }
...
int main()
{
  ...
  try {
    ...
    std::cout << "Extrinsics cMd: \n"
              << rs.getTransformation(rs::stream::color, rs::stream::depth)
              << std::endl;
    std::cout << "Extrinsics dMc: \n"
              << rs.getTransformation(rs::stream::depth, rs::stream::color)
              << std::endl;
    std::cout << "Extrinsics cMi: \n"
              << rs.getTransformation(rs::stream::color, rs::stream::infrared)
              << std::endl;
    std::cout << "Extrinsics dMi: \n"
              << rs.getTransformation(rs::stream::depth, rs::stream::infrared)
              << std::endl;

    vpImage<vpRGBa> color(
        (unsigned int)rs.getIntrinsics(rs::stream::color).height,
        (unsigned int)rs.getIntrinsics(rs::stream::color).width);

    vpImage<unsigned char> infrared_display(
        (unsigned int)rs.getIntrinsics(rs::stream::infrared).height,
        (unsigned int)rs.getIntrinsics(rs::stream::infrared).width);
    vpImage<uint16_t> infrared_y16;
    rs::device *dev = rs.getHandler();
    bool use_infrared_y16 =
        dev->get_stream_format(rs::stream::infrared) == rs::format::y16;

    vpImage<vpRGBa> depth_display(
        (unsigned int)rs.getIntrinsics(rs::stream::depth).height,
...
#ifdef USE_THREAD
    vpThread thread_display_pointcloud(displayPointcloudFunction,
                                       (vpThread::Args)&pointcloud);
#else
    pcl::visualization::PCLVisualizer::Ptr viewer(
        new pcl::visualization::PCLVisualizer("3D Viewer"));
    pcl::visualization::PointCloudColorHandlerRGBField<pcl::PointXYZRGB> rgb(
    ...
    vpDisplayX di(infrared_display, (int)color.getWidth() + 80, 10,
                  "Infrared image");
    ...
    while (1) {
      ...
      static bool update = false;
      if (!update) {
        viewer->addPointCloud<pcl::PointXYZRGB>(pointcloud, rgb,
                                                "sample cloud");
        viewer->setPointCloudRenderingProperties(
            pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 1, "sample cloud");
        ...
        viewer->updatePointCloud<pcl::PointXYZRGB>(pointcloud, rgb,
                                                   "sample cloud");
      }
      ...
      if (vpDisplay::getClick(color, false) ||
          vpDisplay::getClick(infrared_display, false) ||
