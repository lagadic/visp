...
int main() {
  try {
    ...
    std::cout << rs.getCameraParameters(RS2_STREAM_COLOR, vpCameraParameters::perspectiveProjWithoutDistortion) << std::endl;
    std::cout << rs.getCameraParameters(RS2_STREAM_COLOR, vpCameraParameters::perspectiveProjWithDistortion) << std::endl;
    ...
    vpImage<vpRGBa> color((unsigned int) rs.getIntrinsics(RS2_STREAM_COLOR).height, (unsigned int) rs.getIntrinsics(RS2_STREAM_COLOR).width);
    vpImage<unsigned char> infrared((unsigned int) rs.getIntrinsics(RS2_STREAM_INFRARED).height, (unsigned int) rs.getIntrinsics(RS2_STREAM_INFRARED).width);
    vpImage<vpRGBa> depth_display((unsigned int) rs.getIntrinsics(RS2_STREAM_DEPTH).height, (unsigned int) rs.getIntrinsics(RS2_STREAM_DEPTH).width);
    ...
    while (true) {
...
#ifdef VISP_HAVE_PCL
      {
        ...
        rs.acquire( (unsigned char *) color.bitmap, (unsigned char *) depth.bitmap, NULL, pointcloud_color, (unsigned char *) infrared.bitmap );
      ...
      if (vpDisplay::getClick(color, false) || vpDisplay::getClick(infrared, false) || vpDisplay::getClick(depth_display, false)) {
        break;
      }
