...
std::array<double, 7> add(const std::array<double, 7> &a,
...
std::array<double, 7> subtract(const std::array<double, 7> &a,
                               const std::array<double, 7> &b)
{
  std::array<double, 7> result;
  for (size_t i = 0; i < a.size(); i++) {
...
bool calculateDesiredValues(double t, const std::array<double, 7> &delta_q,
...
int main(int argc, char **argv)
{
  if (argc != 10) {
    std::cerr << "Usage: ./generate_joint_pose_motion "
              << "<robot-hostname> <goal-position> <speed-factor>"
              << std::endl
  ...
  try {
    ...
    calculateSynchronizedValues(delta_q, dq_max, ddq_max_start, ddq_max_goal,
                                &dq_max_sync, &t_1_sync, &t_2_sync, &t_f_sync,
                                &q_1);
    robot.control(
        [=, &time](const franka::RobotState &,
          ...
          bool motion_finished =
              calculateDesiredValues(time, delta_q, dq_max_sync, t_1_sync,
                            ...
                            std::array<double, 7> *delta_q_d)
{
  ...
  for (size_t i = 0; i < 7; i++) {
    ...
    if (std::abs(delta_q[i]) < kDeltaQMotionFinished) {
      ...
      if (t < t_1[i]) {
        (*delta_q_d)[i] = -1.0 / std::pow(t_1[i], 3) * dq_max[i] *
                          sign_delta_q[i] * (0.5 * t - t_1[i]) *
                          std::pow(t, 3);
        ...
        (*delta_q_d)[i] =
            delta_q[i] +
            0.5 *
                (1.0 / std::pow(delta_t_2[i], 3) *
                     (t - t_1[i] - 2 * delta_t_2[i] - t_d[i]) *
                                 ...
                                 std::array<double, 7> *q_1)
{
  ...
  for (size_t i = 0; i < 7; i++) {
    sign_delta_q[i] = sgn(delta_q[i]);
    if (std::abs(delta_q[i]) > kDeltaQMotionFinished) {
      if (std::abs(delta_q[i]) <
          (3.0 / 4.0 * (std::pow(dq_max[i], 2) / ddq_max_start[i]) +
           3.0 / 4.0 * (std::pow(dq_max[i], 2) / ddq_max_goal[i]))) {
        dq_max_reach[i] = std::sqrt(4.0 / 3.0 * delta_q[i] * sign_delta_q[i] *
      ...
      t_f[i] = t_1[i] / 2.0 + delta_t_2[i] / 2.0 +
               std::abs(delta_q[i]) / dq_max_reach[i];
    }
  }
  ...
  for (size_t i = 0; i < 7; i++) {
    if (std::abs(delta_q[i]) > kDeltaQMotionFinished) {
      ...
      (*t_f_sync)[i] = (*t_1_sync)[i] / 2 + delta_t_2_sync[i] / 2 +
                       std::abs(delta_q[i] / (*dq_max_sync)[i]);
      (*t_2_sync)[i] = (*t_f_sync)[i] - delta_t_2_sync[i];
      (*q_1)[i] =
          (*dq_max_sync)[i] * sign_delta_q[i] * (0.5 * (*t_1_sync)[i]);
    }
  }
}
