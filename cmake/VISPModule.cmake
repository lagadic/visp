...
macro(vp_create_module)
  ...
  # Filter out all libs from MacOS SDK path
  set(__argn "")
  vp_get_apple_sdk_dir(apple_sdk_dir)
  foreach(arg ${ARGN})
    vp_string_starts_with(${arg} ${apple_sdk_dir} apple_sdk_dir_found)
    if (NOT apple_sdk_dir_found)
      list(APPEND __argn "${arg}")
    endif()
  endforeach()

  set(VISP_MODULE_${the_module}_LINK_DEPS "${VISP_MODULE_${the_module}_LINK_DEPS};${__argn}" CACHE INTERNAL "")
  _vp_create_module()
        ...
        if(${__to_exclude_from_sources} EQUAL -1)
          ...
          if(${__to_exclude_from_ctest} EQUAL -1)
            ...
            elseif(${t} MATCHES "catch*")
              add_test(${the_target} ${the_target} ${LONG_OPTION_TO_DISABLE_DISPLAY})
            else()
              add_test(${the_target} ${the_target} -c ${SHORT_OPTION_TO_DISABLE_DISPLAY})
            endif()
          endif()
