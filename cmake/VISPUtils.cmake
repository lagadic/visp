...
macro(vp_list_remove_directory lst)
  if(DEFINED ${lst})
    set(_tmp_list ${${lst}})
    foreach(item IN LISTS _tmp_list)
...
#          [VERIFY <condition>])
macro(VISP_OPTION variable description value)
  set(__value ${value})
  set(__condition "")
  set(__verification)
  ...
  if(${__condition})
    ...
    if(DEFINED ${variable} AND "${${variable}}"  # emit warnings about turned ON options only.
        AND NOT (VISP_HIDE_WARNING_UNSUPPORTED_OPTION OR "$ENV{VISP_HIDE_WARNING_UNSUPPORTED_OPTION}")
    )
      message(WARNING "Unexpected option: ${variable} (=${${variable}})\nCondition: IF (${__condition})")
    endif()
    if(VISP_UNSET_UNSUPPORTED_OPTION)
      unset(${variable} CACHE)
    endif()
  endif()
  if(__verification)
    set(VISP_VERIFY_${variable} "${__verification}") # variable containing condition to verify
    list(APPEND VISP_VERIFICATIONS "${variable}") # list of variable names (WITH_XXX;WITH_YYY;...)
  endif()
  unset(__condition)
  unset(__value)
endmacro()

# Check that each variable stored in VISP_VERIFICATIONS list
# is consistent with actual detection result (stored as condition in VISP_VERIFY_...) variables
function(vp_verify_config)
  set(broken_options)
  foreach(var ${VISP_VERIFICATIONS})
    set(evaluated FALSE)
    if(${VISP_VERIFY_${var}})
      set(evaluated TRUE)
    endif()
    message(STATUS "Verifying ${var}=${${var}} => '${VISP_VERIFY_${var}}'=${evaluated}")
    if (${var} AND NOT evaluated)
      list(APPEND broken_options ${var})
      message(WARNING
        "Option ${var} is enabled but corresponding dependency "
        "have not been found: \"${VISP_VERIFY_${var}}\" is FALSE")
    elseif(NOT ${var} AND evaluated)
      list(APPEND broken_options ${var})
      message(WARNING
        "Option ${var} is disabled or unset but corresponding dependency "
        "have been explicitly turned on: \"${VISP_VERIFY_${var}}\" is TRUE")
    endif()
  endforeach()
  if(broken_options)
...
macro(vp_check_compiler_flag LANG FLAG RESULT)
  ...
  set(_fname "")
  set(_fname_is_given FALSE)
  foreach(_param ${ARGN})
    if("${_param}" STREQUAL "FORCE")
      set(_force_check TRUE)
    else()
      set(_fname "${_param}")
      set(_fname_is_given TRUE)
    endif()
  endforeach()
  if(NOT DEFINED ${RESULT} OR _force_check)
    if(_fname)
      # nothing
    ...
    if(_fname)
      if(_fname_is_given)
        file(RELATIVE_PATH __msg "${CMAKE_SOURCE_DIR}" "${_fname}")
...
macro(vp_get_interface_include_dirs tgts output_var)
  foreach(tgt ${${tgts}})
    if(TARGET ${tgt})
      get_target_property(imported_incs_ ${tgt} INTERFACE_INCLUDE_DIRECTORIES)
      if(imported_incs_)
        list(APPEND ${output_var} ${imported_incs_})
      endif()
    endif()
  endforeach()
  vp_list_unique(${output_var})
endmacro()

# A list of libs
macro(vp_get_interface_link_libraries tgts output_var)
  set(__imported_libs)
  set(__libs)
  foreach(tgt ${${tgts}})
    if(TARGET ${tgt})
      get_target_property(imported_libs_ ${tgt} INTERFACE_LINK_LIBRARIES)
      if(imported_libs_)
        foreach(imp_ ${imported_libs_})
          if(NOT imp_ MATCHES "^\\$<")
            list(APPEND __imported_libs ${imported_libs_})
            list(APPEND __libs ${tgt})
          endif()
        endforeach()
  ...
  set(__config "NONE" "RELEASE" "DEBUG" "RELWITHDEBINFO")
  foreach(config_ ${__config})
    foreach(lib_ ${__libs})
      if(TARGET ${lib_})

        get_target_property(imported_libs_ ${lib_} IMPORTED_IMPLIB_${config_})
        if(NOT imported_implib_)
          get_target_property(imported_implib_ ${lib_} IMPORTED_IMPLIB)
        endif()
        if(NOT EXISTS ${imported_libs_})
          get_target_property(lib_location_ ${lib_} IMPORTED_LOCATION_${config_})
          if(NOT lib_location_)
            get_target_property(lib_location_ ${lib_} IMPORTED_LOCATION)
          endif()
        endif()

        set(final_lib_path "")
        if(EXISTS "${imported_implib_}")
          set(final_lib_path "${imported_implib_}")
        elseif(EXISTS "${lib_location_}")
          set(final_lib_path "${lib_location_}")
        endif()

        if(final_lib_path)
          if(WIN32 AND "${config_}" MATCHES "RELEASE|RELWITHDEBINFO|MINSIZEREL")
            list(APPEND ${output_var} optimized "${final_lib_path}")
          elseif(WIN32 AND "${config_}" MATCHES "DEBUG")
            list(APPEND ${output_var} debug "${final_lib_path}")
          else()
            list(APPEND ${output_var} "${final_lib_path}")
          endif()
        endif()
          ...
          foreach(deps_ ${lib_deps_})
            ...
            if(EXISTS "${deps_location_}")
              if(WIN32 AND "${config_}" MATCHES "RELEASE")
                list(APPEND ${output_var} optimized ${deps_location_})
              elseif(WIN32 AND "${config_}" MATCHES "DEBUG")
                list(APPEND ${output_var} debug ${deps_location_})
              else()
                list(APPEND ${output_var} ${deps_location_})
              endif()
            endif()
          endforeach()
        endif()
  ...
  vp_list_remove_directory(${output_var})
...
macro(vp_filter_libraries_with_imported_location libs)
  ...
  foreach(lib_ ${${libs}})
    if(TARGET ${lib_})
      get_target_property(imported_libs_ ${lib_} IMPORTED_LOCATION)
      if(imported_libs_)
...
function(vp_find_dataset found location version major minor patch)
  ...
  if(_found)
    ...
    elseif(EXISTS "${_location}/npz/numpy/visp_npz_test_data_numpy_BE.npz")
      ...
      set(_minor "7")
      set(_patch "2")
    elseif(EXISTS "${_location}/mbt-depth/castel/castel/depth_image_0000.pfm")
      set(_major "3")
      set(_minor "7")
      set(_patch "1")
    ...
    if(inc_dirs)
      ...
      if(_inc_dirs)
        ...
        foreach(_inc_dir ${_inc_dirs})
          if(EXISTS ${_inc_dir})
            list(APPEND _inc_dirs_ok ${_inc_dir})
          endif()
        endforeach()
        set(${inc_dirs} "${_inc_dirs_ok}" CACHE INTERNAL "")
      endif()
    endif()
    if(libs)
      ...
      if(_cfgs)
        foreach(_cfg ${_cfgs})
          ...
          get_target_property(_imp_lib ${name} IMPORTED_IMPLIB_${_cfg_up})
          if(_imp_lib)
          ...
          if(_link_libs)
            ...
            foreach(_link_lib ${_link_libs})
              if(TARGET ${_link_lib})
                get_target_property(_imp_lib ${_link_lib} IMPORTED_IMPLIB_${_cfg_up})
                if(_imp_lib)
                  vp_list_unique(_imp_lib)
                  set(${libs} "${${libs}};${_imp_lib}" CACHE INTERNAL "")
                else()
                  get_target_property(_imp_lib ${_link_lib} IMPORTED_LOCATION_${_cfg_up})
                  if(_imp_lib)
                    vp_list_unique(_imp_lib)
                    set(${libs} "${${libs}};${_imp_lib}" CACHE INTERNAL "")
                  endif()
                ...
                set(${libs} "${${libs}};${_link_lib}" CACHE INTERNAL "")
              endif()
            endforeach()
          endif()
