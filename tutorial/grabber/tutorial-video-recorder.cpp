...
int main(int argc, const char *argv[])
{
  ...
  try {
    ...
    for (;;) {
      ...
      vpDisplay::display(I);
      if (recording == false) {
        vpDisplay::displayText(I, 10, 10, "A click to start recording",
                               vpColor::green);
        ...
        vpDisplay::displayText(
            I, 10, 10, "Recording: A click to stop and exit", vpColor::red);
        if (vpDisplay::getClick(I, false))
          break;
      }
    ...
    std::cout << "The video was recorded in \"" << opt_videoname << "\""
              << std::endl;
  } catch (vpException &e) {
