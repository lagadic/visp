...
void learnCube(const vpImage<unsigned char> &I, vpMbEdgeTracker &tracker,
               vpKeyPoint &keypoint_learning, int id)
{
  ...
  vpKeyPoint::compute3DForPointsInPolygons(cMo, cam, trainKeyPoints, polygons,
                                           roisPt, points3f);
  //! [Keypoints selection on faces]
  ...
  for (std::vector<cv::KeyPoint>::const_iterator it = trainKeyPoints.begin();
...
#else
    cv::Ptr<cv::ORB> orb_learning =
        keypoint_learning.getDetector("ORB").dynamicCast<cv::ORB>();
...
//! [Keypoint declaration]
     ...
     */
    std::string imageName[] = {"cube0001.png", "cube0150.png",
                               "cube0200.png"};
    vpHomogeneousMatrix initPoseTab[] = {
        vpHomogeneousMatrix(0.02143385294, 0.1098083886, 0.5127439561,
                            2.087159614, 1.141775176, -0.4701291124),
        vpHomogeneousMatrix(0.02651282185, -0.03713587374, 0.6873765919,
                            2.314744454, 0.3492296488, -0.1226054828),
        vpHomogeneousMatrix(0.02965448956, -0.07283091786, 0.7253526051,
                            2.300529617, -0.4286674806, 0.1788761025)};
    for (int i = 0; i < 3; i++) {
      ...
      vpDisplay::displayText(
          I, 10, 10,
          "Learning step: keypoints are detected on visible cube faces",
          vpColor::red);
      if (i < 2) {
        vpDisplay::displayText(I, 30, 10, "Click to continue the learning...",
                               vpColor::red);
      } else {
        vpDisplay::displayText(I, 30, 10,
                               "Click to continue with the detection...",
                               vpColor::red);
      }
    ...
    cv::Ptr<cv::ORB> orb_detector =
        keypoint_detection.getDetector("ORB").dynamicCast<cv::ORB>();
    orb_detector =
        keypoint_detection.getDetector("ORB").dynamicCast<cv::ORB>();
    ...
    while (!g.end()) {
      ...
      if (keypoint_detection.matchPoint(I, cam, cMo, error, elapsedTime)) {
        ...
        std::vector<vpImagePoint> ransacInliers =
            keypoint_detection.getRansacInliers();
        std::vector<vpImagePoint> ransacOutliers =
            keypoint_detection.getRansacOutliers();
        //! [Get RANSAC inliers outliers]
      ...
      vpDisplay::flush(I);
      vpDisplay::displayText(IMatching, 30, 10, "A click to exit.",
                             vpColor::red);
