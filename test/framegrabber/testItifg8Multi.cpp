/****************************************************************************
 *
 * $Id: testItifg8Multi.cpp,v 1.3 2006-10-13 12:09:54 fspindle Exp $
 *
...
\n\
SYNOPSIS\n\
  %s [-f <fps=0.01-100.0>] [-i <input=0|1|2|3> \n\
		...
		unsigned &scale, unsigned &buffer, bool &display)
{
  ...
  while ((c = vpParseArgv::parse(argc, argv, GETOPTARGS, &optarg)) > 1) {
    ...
    default:
      usage(argv[0], optarg, fps, input, scale, buffer); return false; break;
    }
  }

  if ((c == 1) || (c == -1)) {
    // standalone param or error
    usage(argv[0], NULL, fps, input, scale, buffer);
    cerr << "ERROR: " << endl;
