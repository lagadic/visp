...
/*!
  Remove outliers though Ransac.
*/
void
vpMbtKltPolygon::ransac()
{
  if(nbPointsCur < minNbPoint){
//     std::cout << "Not enough point to launch ransac" << std::endl;
  }
  else{
    double *xinit = new double[nbPointsCur];
    double *yinit = new double[nbPointsCur];
    
    double *xcur = new double[nbPointsCur];
    double *ycur = new double[nbPointsCur];
    unsigned int ind = 0;
    
    //Create tab for homography computation
    std::map<int, vpImagePoint>::const_iterator iter = curPoints.begin();
    for( ; iter != curPoints.end(); iter++){
      int id(iter->first);
      float i_cur(iter->second.get_i()), j_cur(iter->second.get_j());
      double x_cur(0), y_cur(0);
      vpPixelMeterConversion::convertPoint(cam, i_cur, j_cur, x_cur, y_cur);
      xcur[ind] = x_cur;
      ycur[ind] = y_cur;
      
      double x_init(0), y_init(0);
      vpPixelMeterConversion::convertPoint(cam, initPoints[id].get_i(), initPoints[id].get_j(), x_init, y_init);
      xinit[ind] = x_init;
      yinit[ind] = y_init;
      ind++;
    }
    ...
    //compute homography
    vpHomography H;
    vpColVector inliers(nbPointsCur);
    double residu;
    
    try{
//       std::cout << "Launching ransac with " << nbpoints << " points" << std::endl;
//       vpHomography::ransac(nbpoints, xcur, ycur, xinit, yinit, H, inliers, residu, 1000, 3.0/cam.get_px());
      vpHomography::homographyRansac(nbPointsCur, xcur, ycur, xinit, yinit, H, inliers, residu, 80.0*((double)nbPointsCur)/100.0, 5.0/686.0);
      ...
      std::map<int, vpImagePoint>::const_iterator iter2 = curPoints.begin();
      for( ; iter2 != curPoints.end(); iter2++){
        if(inliers[i] == 1.0){
          tmp[iter2->first] = iter2->second;
          nbPointsCur++;
        }
        else{
      ...
      if(nbSupp != 0)
      std::cout << "nb Suppr : " << nbSupp << std::endl;
      curPoints = std::map<int, vpImagePoint>();
      curPoints = tmp;
      if(nbPointsCur >= minNbPoint) enoughPoints = true;
      else enoughPoints = false; 
