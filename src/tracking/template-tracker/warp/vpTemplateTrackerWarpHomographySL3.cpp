...
//find_warp special a SL3 car methode additionnelle ne marche pas (la derivee n est calculable qu en p=0)
// => resout le probleme de maniere compositionnelle
void vpTemplateTrackerWarpHomographySL3::find_warp(const double *ut0,const double *vt0, const double *u, const double *v,int nb_pt,vpColVector& p)
{
  //std::cout<<"find_warp OVERLOADE"<<std::endl;
  ...
  for(int i=0;i<nb_pt;i++)
  {
    ...
    //std::cout<<"find_warp"<<v[i]<<","<<u[i]<<std::endl;
  ...
  do
  {
    ...
    for(int i=0;i<nb_pt;i++)
    {
      ...
      {
        dW[0][ip]=dW_ddp0[i][ip];
        dW[1][ip]=dW_ddp0[i][ip+NbParam];
      }
...
void vpTemplateTrackerWarpHomographySL3::getp_PyramidDown(const vpColVector &p,vpColVector &pdown)
{
...
void vpTemplateTrackerWarpHomographySL3::getp_PyramidUp(const vpColVector &p,vpColVector &pup)
{
  double *u,*v;u=new double[4];v=new double[4];
  //u[0]=0;v[0]=0;u[1]=640;v[1]=0;u[2]=640;v[2]=480;u[3]=0;v[3]=480;
  u[0]=0;v[0]=0;u[1]=160;v[1]=0;u[2]=160;v[2]=120;u[3]=0;v[3]=120;
...
void vpTemplateTrackerWarpHomographySL3::dWarp(const vpColVector &X1,const vpColVector &X2,const vpColVector &/*ParamM*/,vpMatrix &dW)
{
  vpMatrix dhdx(2,3);
  dhdx=0;
...
void vpTemplateTrackerWarpHomographySL3::dWarpCompo(const vpColVector &/*X1*/,const vpColVector &X2,const vpColVector &/*ParamM*/,const double *dwdp0,vpMatrix &dW)
{
  for(unsigned int i=0;i<NbParam;i++)
  {
    dW[0][i]=Denom*((G[0][0]-X2[0]*G[2][0])*dwdp0[i]+(G[0][1]-X2[0]*G[2][1])*dwdp0[i+NbParam]);
    dW[1][i]=Denom*((G[1][0]-X2[1]*G[2][0])*dwdp0[i]+(G[1][1]-X2[1]*G[2][1])*dwdp0[i+NbParam]);
  }
}

void vpTemplateTrackerWarpHomographySL3::Param_inv(const vpColVector &ParamM,vpColVector &ParamMinv)
{ 
  ParamMinv=-ParamM;
} 
