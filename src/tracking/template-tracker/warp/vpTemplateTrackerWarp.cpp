...
void vpTemplateTrackerWarp::get_p_by_click(const vpImage<unsigned char> &I0,const vpImage<unsigned char> &I,vpTemplateTrackerWarp *warp,vpColVector &p,int nb_pt)
{
  double *u1,*v1;
  double *u2,*v2;
  u1=new double[nb_pt];
  ...
  u2=new double[nb_pt];
  v2=new double[nb_pt];
  std::cout<<"cliquez sur pt image 1"<<std::endl;
  for(int i=0;i<nb_pt;i++)
  {
    vpImagePoint ptclic;
    vpDisplay::getClick(I0,ptclic,true);
    u1[i]=ptclic.get_u();
    v1[i]=ptclic.get_v();
    vpDisplay::displayCross (I0, ptclic, 10, vpColor::red, 2);
    vpDisplay::flush(I0) ;
  }
  std::cout<<"cliquez sur pt image 2"<<std::endl;
  for(int i=0;i<nb_pt;i++)
  {
    vpImagePoint ptclic;
    vpDisplay::getClick(I,ptclic,true);
    u2[i]=ptclic.get_u();
    v2[i]=ptclic.get_v();
    vpDisplay::displayCross (I, ptclic, 10, vpColor::green, 2);
    vpDisplay::flush(I) ;
  }
